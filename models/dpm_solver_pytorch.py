...
class NoiseScheduleVP:
    def __init__(
            ...
            betas=None,
            alphas_cumprod=None,
            continuous_beta_0=0.0001,
            continuous_beta_1=0.02,
            dtype=torch.float32,
        ):
...
def model_wrapper(
    ...
    tex_condition=None,
    pose_condiion=None,
    unconditional_tex=None,
    unconditional_pose=None,
    sem_oimage=None,
    tex_scale=4.0,
    pose_scale=2.0,
    classifier_fn=None,
    ...
    def noise_pred_fn(x, t_continuous, cond=None,pose=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, **model_kwargs)
        elif pose is None and cond is not None:
            output = model(x, t_input, cond, **model_kwargs)
        else:
            output = model(x, t_input, cond, pose,**model_kwargs)
            print('pose')
        if model_type == "noise":
            return output
    ...
    def cond_grad_fn(x, t_input):
        ...
        with torch.enable_grad():
            ...
            log_prob = classifier_fn(x_in, t_input, tex_condition, **classifier_kwargs)
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            return noise - tex_scale * sigma_t * cond_grad
        elif guidance_type == "classifier-free":
            if tex_scale == 1. or unconditional_tex is None:
                return noise_pred_fn(x, t_continuous, cond=tex_condition)
            else:
                t = get_model_input_time(t_continuous)

                e_style_pose = model(x =x, timesteps = t,oimage=tex_condition,pose=pose_condiion,sem_oimage=sem_oimage)

                e_unstyle_pose = model(x=x, timesteps=t, oimage=unconditional_tex, pose=pose_condiion, sem_oimage=sem_oimage)

                e_t_uncond = model(x =x, timesteps = t,oimage=unconditional_tex,pose=unconditional_pose,sem_oimage=sem_oimage)

                e_t = e_t_uncond + 4 * (e_unstyle_pose - e_t_uncond)+3.0*(e_style_pose-e_unstyle_pose)
